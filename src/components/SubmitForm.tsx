    ...
    return (
        <div className="relative flex flex-col justify-center w-full max-w-[800px] px-5 items-center text-white font-thin">
            <form onSubmit={onSubmit} className="flex flex-col gap-4 w-full">
                ...
                <button type="submit" disabled={result == t('CONTACT_FORM_RESPONSE_PENDING') ? true : false} className={`text-center w-full bg-accent px-5 py-3 rounded-md shadow-md hover:bg-secondary hover:text-accent md:transition-all disabled:opacity-30 disabled:hover:bg-accent disabled:hover:text-white disabled:cursor-not-allowed`}>{t('CONTACT_FORM_BUTTON')}</button>

            </form>
            <p className="absolute left-1/2 -translate-x-1/2 bottom-[-40px] w-full text-accent text-center mx-auto">{result} {result == t('CONTACT_FORM_RESPONSE_PENDING') || result == '' ? null :  (<i className={`fa-solid fa-circle-${result == t('CONTACT_FORM_RESPONSE_SUCCESS') ? 'check' : 'xmark'}`}></i>)}</p>
